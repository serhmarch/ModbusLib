...
ServerResource::ServerResource(Port *port, Interface *device) : ServerPort(device)
{
...
Type ServerResource::type() const
{
    return m_port->type();
}

StatusCode ServerResource::open()
{
    m_cmdClose = false;
    return Status_Good;
}

StatusCode ServerResource::close()
{
    m_cmdClose = true;
    return Status_Good;
}

bool ServerResource::isOpen() const
{
    return m_port->isOpen();
}

StatusCode ServerResource::process()
{
...
StatusCode ServerResource::processInputData(const uint8_t *buff, uint16_t sz)
{
    switch (m_func)
    {
...
StatusCode ServerResource::processDevice()
{
    switch (m_func)
    {
    case MBF_READ_COILS: // Read Coil Status
...
StatusCode ServerResource::processOutputData(uint8_t *buff, uint16_t &sz)
{
    switch (m_func)
    {
    case MBF_READ_COILS: // Read Coil Status
