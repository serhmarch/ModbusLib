...
public:
    ...
    /// \details Returns the native handle value that depends on the OS used. For TCP/UDP it is a socket handle, for serial port - a file handle.
...
public:
    ...
    virtual void setServerMode(bool server);

    /// \details Returns `true` if the port works in synchronous (blocking) mode, `false` otherwise.
...
public:
    ...
    virtual Modbus::StatusCode writeBuffer(uint8_t unit, uint8_t func, const uint8_t *buff, uint16_t szInBuff);
    ...
    virtual Modbus::StatusCode readBuffer(uint8_t &unit, uint8_t &func, uint8_t *buff, uint16_t maxSzBuff, uint16_t *szOutBuff);
