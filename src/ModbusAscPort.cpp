...
StatusCode ModbusAscPort::writeBuffer(uint8_t unit, uint8_t func, const uint8_t *buff, uint16_t szInBuff)
{
    ...
    d->sz = bytesToAscii(ibuff, &d->buff[1], szInBuff + 3);
...
StatusCode ModbusAscPort::readBuffer(uint8_t& unit, uint8_t &func, uint8_t* buff, uint16_t maxSzBuff, uint16_t* szOutBuff)
{
    ...
    if ((d->sz = asciiToBytes(&d->buff[1], ibuff, d->sz-3)) == 0)
        return d->setError(Status_BadAscChar, StringLiteral("ASCII. Bad ASCII symbol"));

    if (lrc(ibuff, d->sz-1) != ibuff[d->sz-1])
    ...
    if (d->sz > maxSzBuff)
        return d->setError(Status_BadReadBufferOverflow, StringLiteral("ASCII. Read-buffer overflow"));
