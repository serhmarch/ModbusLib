...
StatusCode ModbusAscPort::writeBuffer(uint8_t unit, uint8_t func, const uint8_t *buff, uint16_t szInBuff)
{
    ...
    if (szInBuff > szIBuff-3)
        return this->setError(Status_BadWriteBufferOverflow, StringLiteral("Write-buffer overflow"));
    ibuff[0] = unit;
    ibuff[1] = func;
    memcpy(&ibuff[2], buff, szInBuff);
    ibuff[szInBuff + 2] = lrc(ibuff, szInBuff+2);
    d_ptr->sz = bytesToAscii(ibuff, &d_ptr->buff[1], szInBuff + 3);
    d_ptr->buff[0]       = ':' ;  // start ASCII-message character
    d_ptr->buff[d_ptr->sz+1] = '\r';  // CR
    d_ptr->buff[d_ptr->sz+2] = '\n';  // LF
    d_ptr->sz += 3;
    return Status_Good;
}
